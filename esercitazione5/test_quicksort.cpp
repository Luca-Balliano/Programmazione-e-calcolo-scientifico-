...
int main(void){
	...
	for(int i = 0; i < 100; i++) { 
        ...
        //uso l'algoritmo
         quick_sort(test_vec,0,dim_corrente-1);
        if(!is_sorted(test_vec)) //L'operatore ! inverte il booleano resituito da is_sorted
        // !TRUE=FALSE
        {
            std::cerr << "Errore: Quicksort ha fallito sui numeri!\n";
            return EXIT_FAILURE; 
        }
    }
    ...
    quick_sort(test_stringhe,0,9);
    if(!is_sorted(test_stringhe)){
        std::cerr << "Errore: Quicksort ha fallito sulle stringhe!\n";
        return EXIT_FAILURE;
    }

    //Se si arriva qui significa che non si è commesso nessun errore 
    std::cout << "Test Quicksort superato con successo!\n";
    return EXIT_SUCCESS;	
